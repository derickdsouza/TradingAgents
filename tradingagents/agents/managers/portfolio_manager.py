...
**Evidence Scorecard** — populate the structured `scorecard` field by scoring each named category on a -2 (strongly bearish) to +2 (strongly bullish) scale, with 0 meaning insufficient or balanced evidence. Categories: Bull Case, Bear Case, Trend / Technical, Fundamental Quality, Liquidity / Risk, Catalyst Clarity, Macro / Regime, Valuation. For EACH category you must also fill the matching `<category>_rationale` field (e.g. `bull_case_rationale`, `liquidity_risk_rationale`) with ONE sentence (≤30 words) that cites the *specific fact* driving the score: a number, a level, a date, or an evidence-ledger key. Generic phrases like "strong fundamentals" or "weak macro" are NOT acceptable — name the metric and the actual figure (e.g. "FY26 net income ₹14,556 Cr vs FY25 ₹11,302 Cr (+29%)"). Set Confidence (Low / Medium / High) and write a one-to-two-sentence `rating_rationale` showing how the scorecard maps to the final rating. For Buy / Sell, name the specific evidence in `invalidating_evidence` that would downgrade or upgrade the call. For Hold, the `tie_breaker` field is REQUIRED — explain why the evidence is genuinely balanced rather than indecisive, and what would break the balance.
