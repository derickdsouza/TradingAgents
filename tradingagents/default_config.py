...
def _resolve_reports_dir_default() -> str:
    """Pick a stable default for the reports directory.

    Walks up from this file looking for a `pyproject.toml`; if found,
    returns ``<project_root>/reports``. This works for both source
    checkouts (one level up) and venv-installed packages (the climb
    eventually reaches the project root that owns the venv). When no
    project root can be located, falls back to ``~/.tradingagents/
    reports`` so the path is still stable across cwd changes.

    Anchoring on a project marker rather than ``Path.cwd()`` avoids the
    ``reports/reports/`` doubling that occurs when the CLI is invoked
    from inside the existing reports directory.
    """
    here = os.path.dirname(os.path.abspath(__file__))
    for _ in range(8):  # cap the climb to avoid runaway loops
        ...
        if parent == here:
            break
        here = parent
    return os.path.join(_TRADINGAGENTS_HOME, "reports")


DEFAULT_CONFIG = _apply_env_overrides({
    "project_dir": os.path.abspath(os.path.join(os.path.dirname(__file__), ".")),
    "results_dir": os.getenv("TRADINGAGENTS_RESULTS_DIR", os.path.join(_TRADINGAGENTS_HOME, "logs")),
    "reports_dir": os.getenv("TRADINGAGENTS_REPORTS_DIR", _resolve_reports_dir_default()),
